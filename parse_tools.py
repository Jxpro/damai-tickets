...
def save_cookies(login_cookies):
    """ 保存cookies """
    with open('__cookies.pkl', 'wb') as fw:
...
def load_cookies():
    """ 读取保存的cookies """
    try:
        with open('__cookies.pkl', 'rb') as fr:
...
def account_login():
    ...
    driver = webdriver.Chrome(os.path.join(os.getcwd(), 'chromedriver'), options=option)
